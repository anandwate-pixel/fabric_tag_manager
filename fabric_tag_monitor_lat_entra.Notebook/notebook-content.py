...
# Only needed if alerting.method='activator' and alerting.activator.auth_method='entra_id'
# (uncomment - not installed by default so the SAS/Graph/SMTP-only path stays lighter):
# %pip install azure-eventhub azure-identity --quiet
...
def _generate_sas_token(resource_uri: str, key_name: str, key: str, ttl_seconds: int = 3600) -> str:
    '''Standard Azure Event Hubs SAS token generation (same algorithm the
    azure-eventhub SDK uses internally).'''
...
def _get_activator_sas_key(cfg: Dict[str, Any]) -> str:
    '''Pulls the Eventstream Custom App SAS key from Key Vault - same pattern
    as the service principal secret, never stored in the notebook/config.'''
    if notebookutils is None:
    ...
    return notebookutils.credentials.getSecret(kv_url, secret_name)


def build_activator_event(cfg, resource, resource_type, missing_tags, owner_email, workspace=None, subject="", body=""):
    '''Flattened, scalar-field event payload - Activator's Detect conditions
    and email templates work on flat properties, so avoid nested objects.'''
...
def send_event_activator_sas(cfg: Dict[str, Any], event: Dict[str, Any]) -> bool:
    '''Publishes via a SAS token. See publish_activator_event() for the
    method-selection entry point actually used by alert_if_missing_tags.

    WIRE-FORMAT NOTE: verify the exact POST body shape (raw JSON vs
    {"body": "<json>"}) against the live sample Fabric shows you in the
    Eventstream's Custom App connection details pane - toggle
    alerting.activator.envelope_format if your tenant expects the wrapped
    form.'''
    ...
    if not (200 <= resp.status_code < 300):
        ...
        return False
    logger.info("Published tag-violation event to Activator Eventstream via SAS for resource '%s' (%s)",
                event.get("resource_name"), event.get("resource_id"))
    return True


def send_event_activator_entra_id(cfg: Dict[str, Any], sp: ServicePrincipal, event: Dict[str, Any]) -> bool:
    '''Publishes via the official Azure SDK, authenticated as the same
    service principal already used for the Fabric Admin API - no SAS key
    involved. Requires: pip install azure-eventhub azure-identity (add a
    cell near the top's %pip install line, or install here inline); the
    tenant setting "Service principals can use Fabric APIs" enabled; and
    the service principal added to the Eventstream's workspace as Member
    or higher.'''
    try:
        from azure.eventhub import EventData, EventHubProducerClient
        from azure.identity import ClientSecretCredential
    except ImportError as e:
        logger.error("alerting.activator.auth_method='entra_id' requires 'azure-eventhub' and "
                     "'azure-identity' (%%pip install azure-eventhub azure-identity): %s", e)
        return False
    ...
    try:
        credential = ClientSecretCredential(tenant_id=sp.tenant_id, client_id=sp.client_id,
                                             client_secret=sp.client_secret)
        producer = EventHubProducerClient(fully_qualified_namespace=namespace, eventhub_name=event_hub_name,
        ...
        logger.error("Failed to publish Activator event via Entra ID auth: %s", e)
        return False

    logger.info("Published tag-violation event to Activator Eventstream via Entra ID auth for resource '%s' (%s)",
                event.get("resource_name"), event.get("resource_id"))
    return True


def publish_activator_event(cfg: Dict[str, Any], sp: ServicePrincipal, event: Dict[str, Any]) -> bool:
    '''Dispatches to the SAS or Entra ID auth path based on
    alerting.activator.auth_method (default 'sas').'''
    auth_method = cfg["alerting"]["activator"].get("auth_method", "sas")
    if auth_method == "sas":
        return send_event_activator_sas(cfg, event)
    if auth_method == "entra_id":
        return send_event_activator_entra_id(cfg, sp, event)
    logger.error("Unknown alerting.activator.auth_method '%s' (expected 'sas' or 'entra_id')", auth_method)
    return False
...
def send_alert_email(cfg: Dict[str, Any], sp: ServicePrincipal, to_address: str, subject: str, body: str) -> bool:
    '''Dispatcher for the two direct-send methods. alerting.method="activator"
    is handled separately in alert_if_missing_tags, since it needs the full
    resource/workspace context to build the event, not just (to, subject, body).'''
    ...
    logger.error("Unknown alerting.method '%s' (expected 'graph', 'smtp', or 'activator')", method)
    return False
...
def alert_if_missing_tags(cfg, sp, resource, resource_type, missing_tags, store, workspace=None):
    ...
    if cfg["alerting"].get("enabled", True) and owner:
        if method == "activator":
            ...
            sent = publish_activator_event(cfg, sp, event)
        else:
